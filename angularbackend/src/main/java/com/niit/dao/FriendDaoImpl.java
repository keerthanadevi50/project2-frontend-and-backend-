...
private SessionFactory sessionFactory;
	...
	public List<User> getSuggestedUsers(User user) {
		...
		SQLQuery query=session.createSQLQuery("select * from user_detail where username in (select username from user_detail where username!=? minus (select from_id from friend_details where to_id=?"
				+ "union select to_id from friend_details where from_id=?"
				+ "))");
	...
	public void updatePendingRequest(String from, String username, char status) {
		Session session=sessionFactory.openSession();
		SQLQuery query=session.createSQLQuery("update friend_details set status=? where from_id=? and to_id=?");
		Transaction trans=session.beginTransaction();
	...
	public List<Friend> listOfFriends(String username){
		Session session=sessionFactory.openSession();
		Transaction trans=session.beginTransaction();
		SQLQuery query=session.createSQLQuery("select * from friend_details where (from_id=? or to_id=?) and status=?");
